...
function showNotification(message, type = "info", title = null) {
  ...
  Swal.fire({
    ...
    customClass: {
      // Clases opcionales para compatibilidad con Tailwind dark mode
  ...
  console.log(
    `[Notification] Type: ${type}, Title: ${titleText}, Message: ${message}`
  );
}
  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        console.warn(
          `[Fetch] Server error response body for ${url}:`,
          errorJson
        );
      } catch (e) {
        // Si el cuerpo no es JSON o está vacío, usa el mensaje HTTP
        console.warn(
          `[Fetch] Non-JSON or empty error response body for ${url}. Status: ${response.status}`
        );
      }
    ...
    if (error.code !== 401 && error.code !== 403) {
      // Evita notificaciones duplicadas si el backend ya envió error específico
      showNotification(
        `Error de comunicación con el servidor: ${error.message || "Intente más tarde."}`,
        "error"
      );
    }
...
function populateForm(form, data) {
  ...
  console.log(
    "[PopulateForm] Rellenando formulario:",
    form.id,
    "con datos:",
    data
  );
  for (const key in data) {
    ...
    if (Object.hasOwn(data, key)) {
      ...
      if (field) {
        ...
        console.warn(
          `[PopulateForm] Campo no encontrado en el formulario ${form.id} para la clave: ${key}`
        );
      }
    }
  }
...
function handleFormSubmit(formSelector, phpScriptEndpoint, successCallback) {
  ...
  if (!form) {
    console.warn(
      `[HandleFormSubmit] Formulario no encontrado: ${formSelector}`
    );
    return;
  }
  ...
  form.addEventListener("submit", async (event) => {
    ...
    console.log(
      `%c[Submit] Iniciando envío para ${formSelector} -> ${phpScriptEndpoint}`,
      "color:orange;"
    );
    ...
    try {
      const data = await fetchData(phpScriptEndpoint, {
        method: "POST",
        body: formData,
      });
      ...
      if (data?.success) {
        ...
        // Caso raro: success: false pero fetchData no lanzó error (ej. error de lógica backend sin código HTTP de error)
        showNotification(
          data.message || "Ocurrió un error en el servidor.",
          "error"
        );
      }
      // Si data es null (204), no hacemos nada aquí.
    } catch (error) {
      console.error(
        `[Submit] Error capturado en el manejador de ${formSelector}:`,
        error
      );
      showNotification(
...
function initializeCommonUI() {
  ...
  if (darkModeToggle) {
    ...
    console.log(
      `[UI] Tema inicial: ${currentThemeIsDark ? "oscuro" : "claro"}`
    );
    ...
    console.warn("[UI] Botón de modo oscuro no encontrado.");
  }
  ...
  if (hamburgerBtn && mobileMenu) {
    ...
    console.warn("[UI] Botón hamburguesa o menú móvil no encontrados.");
  }
      ...
      if (href && href.length > 1 && href.startsWith("#")) {
        try {
          ...
          if (targetElement) {
            ...
            console.warn(
              `[UI] Elemento target para scroll suave no encontrado: ${href}`
            );
          }
        } catch (err) {
  ...
  if (fadeInElems.length > 0 && "IntersectionObserver" in window) {
    const observer = new IntersectionObserver(
      ...
      { threshold: 0.1 }
    ); // Activar cuando el 10% sea visible

    fadeInElems.forEach((el) => observer.observe(el));
    console.log(
      `[UI] Observador Fade-in activado para ${fadeInElems.length} elementos.`
    );
  } else {
  ...
  if (urlParams.has("error") && urlParams.get("error") === "unauthorized") {
    ...
    urlParams.has("error") &&
    urlParams.get("error") === "unauthorized_role"
  ) {
...
async function cargarDatosPerfilUsuario() {
  ...
  if (!form) {
    console.error(
      "[Perfil Usuario] Formulario #update-profile-form no encontrado."
    );
    return;
  }
...
async function cargarCitasUsuario() {
  ...
  if (!listElement) {
    console.error("[Perfil Usuario] Lista #appointments-list no encontrada.");
    return;
  }
  ...
  try {
    ...
    if (data?.success && Array.isArray(data.citas)) {
      if (data.citas.length === 0) {
        listElement.innerHTML =
          '<li class="placeholder">No tiene citas programadas.</li>';
      } else {
    ...
    listElement.innerHTML =
      '<li class="placeholder text-red-500 dark:text-red-400">Error de comunicación al cargar citas.</li>';
...
async function cargarListaMedicos() {
  ...
  if (!selectMedico) {
    console.error(
      "[Perfil Usuario] Select #modal-schedule-medico no encontrado."
    );
    return;
  }
  ...
  try {
    ...
    selectMedico.innerHTML =
      '<option value="" disabled selected>Seleccione un médico...</option>'; // Resetear
    ...
    if (data?.success && Array.isArray(data.medicos)) {
      ...
      selectMedico.innerHTML =
        '<option value="" disabled>Error al cargar médicos</option>';
    }
  } catch (error) {
    selectMedico.innerHTML =
      '<option value="" disabled>Error de comunicación</option>';
...
async function cargarHistorialUsuario() {
  ...
  if (!historyList) {
    console.warn(
      "[Perfil Usuario] Lista #history-list no encontrada. Omitiendo carga de historial."
    );
    return;
  }
  ...
  try {
    ...
    if (data?.success && Array.isArray(data.historial)) {
      if (data.historial.length === 0) {
        historyList.innerHTML =
          '<li class="placeholder">No hay registros en tu historial médico.</li>';
      } else {
        data.historial.forEach((registro) => {
          const li = document.createElement("li");
          li.className =
            "mb-3 pb-3 border-b border-gray-200 dark:border-gray-700 text-sm";
          li.innerHTML = `
    ...
    historyList.innerHTML =
      '<li class="placeholder text-red-500 dark:text-red-400">Error de comunicación al cargar historial.</li>';
  }
}
...
function openScheduleModal() {
  ...
  if (!modal) {
    console.error("[Perfil Usuario] Modal #schedule-modal no encontrado.");
    return;
  }
...
async function cargarDatosPerfilMedico() {
  ...
  if (!form) {
    console.error(
      "[Perfil Médico] Formulario #doctor-profile-form no encontrado."
    );
    return;
  }
  setLoadingState(form, true, "Cargando Perfil...");
...
function crearElementoCitaMedico(cita) {
  ...
  const indicadorHoy = esHoy
    ? '<span class="ml-2 text-xs font-bold text-red-600 dark:text-red-400">[HOY]</span>'
    : "";
  ...
  li.innerHTML = `
        ...
        <div class="mt-4 flex flex-wrap gap-2 justify-start">
            ...
            ${
              cita.estado !== "Cancelada Paciente" &&
              cita.estado !== "Cancelada Doctor" &&
...
async function cargarCitasMedico() {
  ...
  if (!listElement) {
    console.error(
      "[Perfil Médico] Lista #appointments-list-doctor no encontrada."
    );
    return;
  }
  ...
  try {
    ...
    if (data?.success && Array.isArray(data.citas)) {
      if (data.citas.length === 0) {
        ...
        data.citas.forEach((cita) =>
          listElement.appendChild(crearElementoCitaMedico(cita))
        );
...
async function cargarPacientesHoy() {
  ...
  if (!listElement) {
    console.error("[Perfil Médico] Lista #patients-list-doctor no encontrada.");
    return;
  }
  ...
  try {
    ...
    if (data?.success && Array.isArray(data.pacientes)) {
      if (data.pacientes.length === 0) {
        listElement.innerHTML =
          '<li class="placeholder">No hay pacientes agendados para hoy.</li>';
      } else {
        data.pacientes.forEach((paciente) => {
          const li = document.createElement("li");
          li.className =
            "mb-2 pb-2 border-b dark:border-gray-700 flex justify-between items-center text-sm flex-wrap gap-2";
    ...
    listElement.innerHTML =
      '<li class="placeholder text-red-500 dark:text-red-400">Error de comunicación.</li>';
  }
}
...
async function guardarNotasConsulta(event) {
  ...
  console.log(
    "%c[Panel Médico] Intentando guardar notas de consulta...",
  ...
  if (!window.selectedCitaIdForNotes) {
    // Usar una variable global o un atributo data en el form
    showNotification(
      "Por favor, selecciona una cita ('Cargar p/ Notas') antes de guardar.",
      "warning",
      "Seleccione Cita"
    );
    return;
  ...
  if (!notasTextarea || notasTextarea.value.trim() === "") {
    showNotification(
      "El campo de notas no puede estar vacío.",
      "warning",
      "Notas Requeridas"
    );
  ...
  try {
    const data = await fetchData("Citas/guardar_notas_consulta.php", {
      method: "POST",
      body: formData,
    });
    if (data?.success) {
      ...
      document
        .querySelectorAll(".cita-seleccionada-para-notas")
        .forEach((el) =>
          el.classList.remove(
            "cita-seleccionada-para-notas",
            "bg-blue-100",
            "dark:bg-blue-900"
          )
        );
...
async function cambiarEstadoCita(idCita, nuevoEstado) {
  console.log(
    `%c[Acción Cita] Solicitando: ${nuevoEstado} para cita ${idCita}`,
    "color: teal;"
  );
  ...
  try {
    ...
    const data = await fetchData("Citas/cambiar_estado_cita.php", {
      method: "POST",
      body: formData,
    });
...
function attachCitaActionListeners(containerSelector) {
  ...
  if (!container) {
    console.warn(
      `[Listeners] Contenedor de citas no encontrado: ${containerSelector}`
    );
    return;
  }
  ...
  const handleCitaActionClick = (event) => {
    ...
    console.log(
      `%c[Click Acción Cita] Contenedor=${containerSelector}, Acción=${action}, ID=${idCita}`,
      "color: darkcyan;"
    );
    ...
    const reEnableButton = () => {
      if (action !== "cargar-notas") button.disabled = false;
    };
    ...
    switch (action) {
      ...
      case "cargar-notas":
        ...
        container
          .querySelectorAll("li.cita-seleccionada-para-notas")
          .forEach((li) => {
            li.classList.remove(
              "cita-seleccionada-para-notas",
              "bg-blue-100",
...
document.addEventListener("DOMContentLoaded", function () {
  console.log(
    "%c[MediAgenda] DOM Cargado. Inicializando scripts.js...",
  ...
  if (currentPath.includes("registro.php")) {
    ...
    if (tabs.length > 0 && tabContents.length > 0) {
      ...
      console.warn(
        "[Routing] Elementos de tabs no encontrados en registro.php."
      );
    }
    ...
    handleFormSubmit("#login-form", "Auth/login.php", (data) => {
      if (data?.success && data.rol) {
        ...
        if (!redirectMap[data.rol.toLowerCase()]) {
          showNotification(
            "Rol desconocido. Redirigiendo al inicio.",
            "warning"
          );
        }
    ...
    handleFormSubmit(
      ...
      "Auth/registrar_paciente.php",
      (data, form) => {
        // Éxito: mostrar mensaje y cambiar a la pestaña de login
        showNotification(
          data.message ||
            "Registro de paciente exitoso. Ahora puedes iniciar sesión.",
          "success"
        );
        form.reset();
        const showTabLogin = document.querySelector(
    ...
    handleFormSubmit(
      ...
      "Auth/registrar_medico.php",
      (data, form) => {
        showNotification(
          data.message ||
            "Registro de médico exitoso. Ahora puedes iniciar sesión.",
          "success"
        );
        form.reset();
        const showTabLogin = document.querySelector(
  ...
  // --- Página: Panel Paciente (`perfil-usuario.php`) ---
    ...
    handleFormSubmit(
      ...
      "Citas/programar_cita.php",
      (data, form) => {
        showNotification(
          data.message || "Cita programada con éxito.",
          "success"
        );
        form.reset();
    ...
    if (btnShowSchedule)
      btnShowSchedule.addEventListener("click", openModalHandler);
    if (linkShowSchedule)
      linkShowSchedule.addEventListener("click", openModalHandler);
    if (btnCloseSchedule)
  ...
  // --- Página: Panel Médico (`perfil-doctores.php`) ---
    ...
    if (notesForm) {
      ...
      console.warn(
        "[Panel Médico] Formulario #consulta-notes-form no encontrado."
      );
    }
  }
  ...
  // que ya se carga en esa página. No se necesita lógica adicional aquí en `scripts.js`.
    ...
    console.log(
      "[Routing] Lógica principal para panel-admin-sistema.php se encuentra en panel-admin.js."
    );
  }
  ...
  // --- Otras Páginas (ej. index.php) ---
    ...
    console.log(
      "[Routing] Sin lógica JS específica para ejecutar en esta página."
    );
  }

  console.log(
    "%c[MediAgenda] Inicialización de scripts.js completada.",
    "color: green; font-weight: bold;"
  );
